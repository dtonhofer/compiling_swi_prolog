...
# ===========================================================================
# Getting the Java jars needed for "JPL", the Java<->Prolog bridge. 
# ===========================================================================
#
# Those jars are needed to run the unit test cases. 
#
# The "JUnit 4" jar, providing the test framework:
#
#   Available at:
#
...
#   latest is Junit Jupiter / Junit 5
#
#   Get it directly:
# 
#   > curl https://repo1.maven.org/maven2/junit/junit/4.13.2/junit-4.13.2.jar --output junit-4.13.2.jar
#   > sha256sum junit-4.13.2.jar
#
#   yields: SHA-256 8e495b634469d64fb8acfa3495a065cbacc8a0fff55ce1e31007be4c16dc57d3
#
# The "hamcrest" jar, providing test expressions:
#
#   Available at: 
#
...
#   latest is Hamcrest 3.0
#
#   Get it directly:
#
#   > curl https://repo1.maven.org/maven2/org/hamcrest/hamcrest/2.2/hamcrest-2.2.jar --output hamcrest-2.2.jar
#   > sha256sum hamcrest-2.2.jar
#
#   yields: SHA-256 5e62846a89f05cd78cd9c1a553f340d002458380c320455dd1f8fc5497a8a1c1
#
# ===========================================================================
...
#
# - Disentangle building the building of the PDF of the manual.
#
...
do_packages_exist() {
   ...
   if [[ $system == fedora || $system == rpm_and_dnf ]]; then
     # "zlib-devel" has been replaced by "zlib-ng-compat-devel"
     for package in \
         cmake \
         ninja-build \
         gcc \
...
message_about_cmake_logfiles() {
   ...
   local build_dir=$1
   local cmake_logfile="CMakeFiles/CMakeOutput.log"
   local cmake_errfile="CMakeFiles/CMakeError.log"
   local file_length
   ...
   if [[ -f "$build_dir/$cmake_errfile" ]]; then
      echo -n "ATTENTION! You may want to inspect CMake error file '$build_dir/$cmake_errfile' ... "
      file_length=$(wc -l "$build_dir/$cmake_errfile" | sed 's/\s.*$//g')
      echo "it has $file_length lines"
   else
      echo "There is no CMake error file '$build_dir/$cmake_errfile' ... good!"
   fi
...
build() {
   ...
   # If the Java<->Prolog bridge 'JPL' shall be compiled, Java jars to run tests should exist.
   # If one skips compiling 'JPL' or skips the tests, these jars are not needed.
   ...
   echo "That directory should contain Java jars to test the Prolog<->Java bridge 'JPL'" >&2
   echo "Checking whether jars are there." >&2
   if [[ -f "$hamcrest_jar_fq" ]]; then
   echo "   The 'hamcrest' jar exists           : $hamcrest_jar_fq" >&2
   else
   echo "   The 'hamcrest' jar IS MISSING       : $hamcrest_jar_fq" >&2
   echo "   get it from: https://mvnrepository.com/artifact/org.hamcrest/hamcrest/" >&2
   fi
   if [[ -f "$junit_jar_fq" ]]; then
   echo "   The 'junit4' jar exists             : $junit_jar_fq"    >&2
   else
   echo "   The 'junit4' jar IS MISSING         : $junit_jar_fq"    >&2
   echo "   get it from: https://mvnrepository.com/artifact/junit/junit/" >&2
   fi
   else
   echo "ATTENTION! There is no jar directory   : $jar_dir_fq"      >&2
   echo "That directory should contain Java jars to test the Prolog<->Java bridge 'JPL'" >&2
   echo "Compilation MAY fail if these are missing." >&2
   fi
   ...
   # From this point onwards, the ninja build file is key: 
   # $toplevel_dir_fq/system/master_swipldevel/build/build.ninja

   ninja || {
      echo "'ninja' command failed -- exiting" >&2
      local ninjabuild_fq="$toplevel_dir_fq/system/master_swipldevel/build/build.ninja"
      if [[ -f $ninjabuild_fq ]]; then
         echo "The ninja build file is at '$ninjabuild_fq', if you want to take a look." >&2
      else 
         echo "...there is no ninja build file at '$ninjabuild_fq', which is unexpected." >&2
      fi
      exit 1
   }
